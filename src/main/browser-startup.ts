...
export async function wakeBrowserUrl(url: string, retry = false, backgroundStartup = false,
  ...
  const prior = waking;
  // A configured --user-data-dir does not give us authority to forward this URL to a running
  // browser. Chromium may hand it to an existing family process, and that can activate the wrong
  // window/profile. Only a confirmed absence of the selected browser family grants a cold-start
  // opening. External subagents that need an already-running dedicated profile use their separate
  // Oracle attach transport rather than weakening this native startup boundary.
